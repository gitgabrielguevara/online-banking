...
const calcDisplaySummary = function (acc) {
  ...
  labelBalance.textContent = formatCur(acc.incomes, acc.locale, acc.currency);

  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelBalance.textContent = formatCur(Math.abs(out), acc.locale, acc.currency);
  ...
  labelBalance.textContent = formatCur(interest, acc.locale, acc.currency);
};
